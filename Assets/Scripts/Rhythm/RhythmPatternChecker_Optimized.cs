...
/// 
/// 최적화 포인트:
/// - GameServices를 통한 참조 관리
/// - 입력 처리 최적화
/// - GC Allocation 최소화
/// - Dictionary 사용 최적화
/// </summary>
public class RhythmPatternChecker_Optimized : MonoBehaviour
{
    #region 컴포넌트 참조 (캐싱)
    ...
    private struct InputEvent
    {
        ...
        public RhythmJudgment judgment;
    }
    ...
    void Start()
    {
        if (RhythmManager != null)
            RhythmManager.OnBeatCounted.AddListener(OnBeat);
        else
    ...
    void ProcessSkillInput(KeyCode key)
    {
        ...
        // Focus 업데이트
        UpdateFocus(judgment);

        // 스킬 시퀀스 처리
        ProcessSkillSequence(key, judgment);
    }

    void ShowJudgmentFeedback(RhythmJudgment judgment)
    {
        if (UI != null)
    ...
    #region 스킬 시퀀스 관리
    void ProcessSkillSequence(KeyCode key, RhythmJudgment judgment)
    {
        ...
        if (_currentActiveSkillKey == KeyCode.None || _currentActiveSkillKey == key)
            ContinueSkillSequence(key, skill);
        else
            StartNewSkillSequence(key, skill);
    }

    void StartNewSkillSequence(KeyCode key, ConstellationSkillData skill)
    {
    ...
    void ContinueSkillSequence(KeyCode key, ConstellationSkillData skill)
    {
    ...
    void ExecuteSkill(ConstellationSkillData skill)
    {
        ...
        switch (skill.category)
        {
            ...
            case SkillCategory.Movement:
                float chargeDistance = skill.inputCount * _playerController.moveDistance;
                _playerController.ActivateCharge(chargeDistance);
                break;
    ...
    void ApplyFlashToClosestGuard(int durationInBeats)
    {
        ...
        GuardRhythmPatrol_Optimized closestGuard = null;
        float minSqrDistance = float.MaxValue;

        for (int i = 0; i < hitCount; i++)
        {
            var guard = results[i].GetComponent<GuardRhythmPatrol_Optimized>();
            if (guard != null)
            {
                float sqrDist = (results[i].transform.position - (Vector3)playerPos).sqrMagnitude;
    ...
    public int GetRemainingCooldown(ConstellationSkillData skill)
    {
        ...
        if (!_skillCooldowns.ContainsKey(skill)) return 0;
    ...
    {
        CleanupExpiredCooldowns(currentBeat);
    }

    void CleanupExpiredCooldowns(int currentBeat)
    {
        // List를 사용하여 제거할 키를 임시 저장 (GC 최소화)
    ...
    void CleanupInputBuffer()
    {
        ...
        {
            InputEvent oldestInput = _inputBuffer.Peek();
            if (currentTime - oldestInput.timestamp > INPUT_BUFFER_TIME)
    ...
    {
        if (RhythmManager != null)
            RhythmManager.OnBeatCounted.RemoveListener(OnBeat);
    }
}